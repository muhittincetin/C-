...
class Klas1 
{
	string ad, soyad, tckn,ilacismi,firmaismi,barkodno; //Degiþkenlerimizi tanimladýk
protected:
	
	ofstream dosyaYaz;
	ifstream dosyaOku;
	
	//SET Fonksiyonlarýmýzý tanýmladýk
	void setad(char ad1);  
	...
	//Ekle fonksiyonlarýmýzý tanýmladýk
	void IlacEkle();
	void PersonelEkle();
	//Listele fonksiyonlarýmýzý tanýmladýk
	void IlacListele();
...
void Klas1::IlacEkle()
{
	string kontrol;
					string a;
					int pl; //Ýsim kýsýmlarýnda rakam ve kullanýlamayan karakterlerin kontrolü için oluþturulan deðiþkenimiz
					do {
						system("cls");
						do {
							pl = 0;
						...
						int u = 0, v = 0;  //tc no nizamimi ve daha önceden kullanýlmýþ mý diye kontrol ederken kullanýlan deðiþkenler.
						char n;
						string tutBarkodNo, tg, yh, th; //dosyadan okuma esnasýnda isimlerin ve barkod no nun karýþmamasý için kullanýlan deðiþkenler.
						do {
							...
							while (dosyaOku >> tg >> th >> barkodno >> yh) {
								if (tutBarkodNo == barkodno)
								{											//eðer barkodno sistemde kayýtlýysa olo deðiþkenine 1 atar.
									v = 1;
								}
							}
							dosyaOku.close();
							...
							for (int i = 0; i < (tutBarkodNo.length()); i++) {
								n = tutBarkodNo[i];
								p = static_cast<int>(n);
								if (!(p > 47 && p < 58))						//eðer barkodno da rakamdan farklý bir karakter kulllanýlmýþsa u deðiþkenine 1 atar.
									u = 1;
							}
							if (u == 1) {
								system("cls");
								...
								system("cls");
								cout << "BARKOD NUMARASI 5 HANELI OLMALI...\n\a";					//barkodno 5 haneli deðilse uyarý verir.
								system("pause");
								system("cls");
							}
							else if (v == 1) {
						...
						} while (u == 1 || tutBarkodNo.length() != 5 || v == 1); //eðer uyarýlardan biri bile olduysa barkodno nun yeniden girilmesini ister.
...
void Klas1::PersonelEkle()
{
	string kontrol;
					...
					do {
						...
						int u = 0, v = 0;  //tc no nizamimi ve daha önceden kullanýlmýþ mý diye kontrol ederken kullanýlan deðiþkenler.
						char n;
						string tutTC, tg, th; //dosyadan okuma esnasýnda ad,soyad ve tcnin karýþmamasý için kullanýlan deðiþkenler.
						do {
							...
							if (u == 1) {
								...
								system("cls");
								cout << "BU TCKN KAYITLARDA BULUNMAKTA. BASKA BIR TCKN GIRINIZ...\n\a";			//v deðikenine 1 atanmýþsa uyarý verir.
								system("pause");
								system("cls");
							}
						} while (u == 1 || tutTC.length() != 11 || v == 1); //eðer uyarýlardan biri bile olduysa tckn nin yeniden girilmesini ister.
...
{
	system("color F4"); //komut ekranýný renklendiriyoruz
	do {
		system("cls");
